...
class GraspEstimatorBase(ABC):
    ...
    def predict_grasps(
        ...
        Args:
            ...
            xyz_object_pc: [M x 3] tensor of xyz point cloud belonging to the selected object.
...
class GraspEstimatorCGN(GraspEstimatorBase):
    """Grasp estimator module based on Contact-GraspNet from <https://arxiv.org/abs/2103.14127>."""
    ...
    def predict_grasps(
        ...
        if self.filter_grasps:
            grasp_indices_f = self._filter_grasps(
                pred["pred_points"][grasp_indices], xyz_object_pc
            )
    ...
    def _preprocess_pc_for_inference(
        ...
        xyz_pc = filter_point_cloud_by_depth(
            xyz_pc, min_d=self.min_depth, max_d=self.max_depth
        )
    ...
    def _predict_grasps(self, xyz_pc: FloatTensor) -> Dict[str, FloatTensor]:
        ...
        xyz_pc, xyz_pc_mean = self._preprocess_pc_for_inference(
            xyz_pc, self.convert_cam_coords
        )

        # Repeat point cloud to perform simulataneous forward passes on same point cloud
        xyz_pc = xyz_pc.repeat(
            (self.forward_passes, 1, 1)
        )  # Shape = (forward_passes, N, 3)
        ...
        pred["pred_widths"] = torch.minimum(
            pred["pred_widths"] + self.gripper_extra_opening,
            torch.tensor(self.gripper_width),
        )
    ...
    def _select_grasps(
        ...
        if n_grasps_total == 0:
            warnings.warn(
                "No grasp predicitions satisfy set confidence threshold contraint."
            )
            return conf_indices
    ...
    def _filter_grasps(
        self, pred_points: FloatTensor, obj_points: FloatTensor
    ) -> IntTensor:
        ...
        dist_sqr = get_squared_distances(
            pred_points.unsqueeze(dim=0),
            obj_points.unsqueeze(dim=0),
        ).squeeze(dim=0)

        # Filter based on minimum distance for each contact point
        dist_sqr = torch.amin(dist_sqr, dim=1)
        grasp_indices = torch.nonzero(
            dist_sqr < self.FILTER_DIST_THRESH, as_tuple=True
        )[0]
        return grasp_indices
